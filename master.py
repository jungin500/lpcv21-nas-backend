...
from framework.picontroller import PiControllerServer
from framework.master.analyzer import PiCurrentAnalyzer
from framework.util import Timer
...
def main():
    ...
    print("== PiControlServer v1.2 by LimeOrangePie ==")
    ...
    ANALYZER_PATH = 'framework/master/bin/Power-Z.exe'
    analyzer = PiCurrentAnalyzer(ANALYZER_PATH)
    analyzer.open()

    timer = Timer()

    model_list = ['shufflenet', 'mb3_ssd_pretrained', 'efficientdet']
    ...
    for model_name in model_list:
        ...
        analyzer.end()
        metric = analyzer.postprocess(model_name, elapsed_time, picontrol.frame_total)
        print("Model %s Metric: %.3f" % (model_name, metric))
